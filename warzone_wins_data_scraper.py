...
for num in range(0, 23309):    # A for loop that counts up to 23309, which is the number of pages of data I need.
    ...
    url = 'https://cod.tracker.gg/warzone/leaderboards/battle-royale/all/Wins?page={}'.format(num)
    res = requests.get(url)
    html_soup = BeautifulSoup(res.text, 'html.parser')

    userAndPlatform = html_soup.find_all('td', class_='username')
    wins = html_soup.find_all('td', class_="stat")
    ...
    for i in range(len(win1)):
        ...
        userWin.append([str(win1[i])[50:].split('>')[1].split('<')[0].replace(',', '')])
    ...
    for h in range(len(userInfo)):
        final.append(userInfo[h] + userWin[h] + userMatchesPlayed[h])

    actualFinal.append(final)

    fmt = '{:<14}{:<18}{:15}{:15}{:15}'

    pp.pprint(fmt.format('User', 'Platform', 'Wins', 'Matches Played', 'Page {}'.format(num)))
    pp.pprint(final)
    #
    # with open('C:/Users/dpowe_4rzki43/OneDrive/Desktop/Warzone_CSV_Files/warzone_wins.csv', 'a+') as outcsv:
